...
function getNextFriday(date) {
  ...
  return day < 5
    ? new Date(date.setDate(today + (5 - day)))
    : new Date(date.setDate(today + (7 - day + 5)));
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  const period = Math.abs(Date.parse(dateEnd) - Date.parse(dateStart));
  return Math.round(period / oneDay) + 1;
}
...
function getWeekNumberByDate(date) {
  ...
  const days = (date - year) / (1000 * 60 * 60 * 24);

  return Math.ceil((days + (year.getDay() || 7)) / 7);
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  while (data <= end) {
    if (workDays < countWorkDays) {
      ...
      workDays = 0;
      data.setDate(data.getDate() + countOffDays - 1);
    }
